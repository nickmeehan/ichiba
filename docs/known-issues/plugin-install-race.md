...
| 2026-08-18 | 2.1.234 | `container_019DLLJT9uS9DMdD16yB4QXW--claude_code_remote--210223` | yes (literal `race_fired=no`, but `failed=4` so the race fired and the workaround lost it again) | Fresh container confirmed (`/tmp/claude-code.log` starts 07:44:35Z, checked ~90s later). vDA sync at 07:44:37.8Z skipped all 4 plugins (`marketplace not registered`), saved 0. `installPluginsForHeadless: starting` didn't fire until **07:45:48.2Z — ~70s after vDA**, the widest gap logged yet — and its reconcile list contained only `claude-plugins-official`; **`ichiba` never appears in `known_marketplaces.json` this session at all** (still logging "orphaned enabledPlugins entry … marketplace not registered" for the `@ichiba` plugins *after* `installPluginsForHeadless` finished). The hook's 0/1/2/4/8s (~15s) backoff expired long before the 70s mark, so `bin/install-enabled-plugins.sh` logged `enabled=4 already=0 installed=0 failed=4`. `installed_plugins.json` stayed `{"version":2,"plugins":{}}`, `claude plugin list` → "No plugins installed", and this session's available-skills list has no `dev-workflow:*`/`docs-kb:*`/`plugin-dev:*` entries (only the built-in bare `skill-creator`). Upstream race is clearly still present in 2.1.234, and the `ichiba`-marketplace omission (not just lateness) looks like a regression vs. the 2.1.150 observations above, where both marketplaces eventually cloned. Could not verify upstream issue status: the task's referenced `docs/known-issues/plugin-install-race-upstream-issue.md` does not exist in this repo (nothing to link/re-file from), and a GitHub search for the exact title turned up no match — closest hit, #19275, is a different, closed/stale issue from another author. Recommend Nick confirm whether an upstream issue was ever actually filed, and investigate why `installPluginsForHeadless` is now dropping the `ichiba` marketplace entirely rather than just running late. |
